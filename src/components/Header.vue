...
<template>
  <div class="h-screen flex flex-col justify-between">
    <header
      ...
      <div
        class="flex flex-col bg-white p-4 lg:mr-20 xl:mr-24 rounded-2xl border-black border-4 neo-shadow overflow-hidden dark:bg-[#4e4e4e]">
      ...
      <div
        class="flex flex-col aspect-[2/2.7] mx-auto bg-yellow-200 dark:bg-[#776e2d] rounded-2xl border-black border-4 neo-shadow px-4 max-w-[18rem] md:max-w-[20rem]">
        <img :src="imageUrl" alt="A picture of Nate"
             class="h-auto w-auto rounded-xl border-black border-4 mt-4">
        <h1 class="subheader text-[1.5rem] lg:text-[2rem]">{{ imageText }}</h1>
      </div>
    </header>
    <Motion as="svg" fill="none" viewBox="0 0 24 24" strokeWidth="5.0"
            stroke="currentColor" class="w-10 h-10 md:w-14 md:h-14 mx-auto animate-bounce"
            :initial="{opacity: 0}"
